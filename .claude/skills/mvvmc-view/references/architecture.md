...
> ⚠️ **效能隔離的前提是子組件為純值——這條上面三項都沒說，而第 3 項正好指向反例。**
>
> 實測（`Experiments/ViewSplitProbe/`，2026-09-08）：清單**只改順序**時，持有 `@State` 的子組件**全部重跑**（`bodies=3`），純值子組件**全部跳過**（`A=0 B2=0 C=0`）。
>
> 所以「拆成獨立 `struct` 就有 diffing 跳過」在**子組件自己持有 `@State`** 時不成立。而第 3 項（夠複雜、有自己的 Action 或子組件）描述的**正是最可能持有 `@State` 的那種組件**（展開、選取、輸入焦點、本地 toggle）——照著三條判準走，會被優先推向那個在重排下失去 skip 的形狀。
>
> 需要子組件持有暫態又要重排 skip 時，把那個暫態提升到父層（以 item id 為 key），讓子組件回到純值。
