...
> ⚠️ **效能隔離曾經有一個前提：子組件必須是純值。這條上面三項都沒說，而第 3 項正好指向反例。**
> **但這個前提在 Xcode 27 SDK 下消失了——而決定你落在哪一邊的是「你用哪個 SDK 建置」，不是「使用者跑哪個 iOS」。**
>
...
>
> `Experiments/ViewSplitProbe/`。2026-09-10 在 Xcode 27 下重測，並用 Xcode 26.4.1 跑對照組，每格各兩次、結果一致：
>
> ```
> Xcode 26.4.1 SDK：
...
>
> Xcode 27 SDK（同一個 iOS 26.4 模擬器）：
> reorder withID  @State survived=true  child bodies=0
> reorder noID    @State survived=true  child bodies=0
> ```
>
> **`survived=true` 在四種組合下全部成立**（兩個 SDK × iOS 26.4／27.0 模擬器），所以本節下面那三項「沒有重現」的結論不但沒被推翻，反而多了一組獨立佐證。變動的只有 `bodies`，那是〈拆與不拆〉那條的事，見本節末。
>
> | 本節說的 | 實測 |
> |---|---|
...
>
> **順帶量到一件 §7 需要的事，而它在 2026-09-10 變了一半**：純值子組件（不持有 `@State`）在**只有順序改變**時會被完整跳過（`skip reordered A=0 B2=0 C=0`）——這一條兩個 SDK 都成立，沒變。變的是持有 `@State` 的那種：Xcode 26 下全部重跑（`bodies=3`），Xcode 27 下全部跳過（`bodies=0`）。
>
> 所以「差別在子組件持不持有 `@State`」**只對 Xcode 26 SDK 成立**。在 Xcode 27 SDK 下兩者行為相同，§7 那張表在重排下無條件成立。完整的 SDK 對照與適用判斷見〈拆與不拆的決策準則〉那條。
