...
>
> **Correction (2026-09-17): the sentence above overstates what was carried back.** Only `mvvmc-hostcontroller` ever received a real "do not file against this skill" exemption. `mvvmc-navigation`'s line is an `Info.plist` pointer, not an exemption, and `mvvmc-view` / `mvvmc-viewmodel` / `mvvmc-model` had none — **1 of 5, not 4 of 5.** The gap that paragraph is describing was larger than the paragraph knew, which makes its own conclusion stronger, not weaker.
>
> **What happened next (2026-09-17): both halves of that reasoning turned out to be wrong, and the document was deleted rather than quarantined.** "Nothing can verify it" was false — the `MVVMC-Skip` fork had validated it three months earlier, and the freeze notice's claim to the contrary went on to mislead a working session into a wrong conclusion. And quarantine was not actually cheaper: a frozen document still has to be maintained, still gets loaded when someone types its name, and still carries the exemptions that five other files must keep pointing at. The real asymmetry was elsewhere — **the same content existed in two genres.** The skill stated it as present-tense rules, which expire silently; the fork states it as a dated Migration Log (22 entries, each with `Why` and `Verification`) plus an Idiom list sitting next to code that compiles, which cannot expire because it never claims to be current. → **Before quarantining a document, check whether a better-genre copy already exists. If it does, the choice is not quarantine-vs-keep but which genre to delete** — and the present-tense one is always the one that rots.
