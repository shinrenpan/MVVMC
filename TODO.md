...
- [ ] **Skip × V-layer Action Pattern** *(blocked: 需要一個「含 `send` 且已完成 Skip 建置」的專案，目前為零——`MVVMC-Skip` 的 baseline `v1.1.0` 早於 `send`（fork 裡 `let send` 0 命中），`FoodEntropy` 有 `send` 但尚未移植。非「還沒想」)* — `mvvmc-skip` 眉角 #6（巢狀 enum 在 Kotlin 要完整限定）目前只涵蓋 `doAction(.view(...))`。V 層的 `send(.rowDidTap)` 同樣是巢狀 enum 的 leading-dot 呼叫，理論上也會踩到，且 `send` 現在的型別是 `@MainActor (Action) -> Void`（Skip 如何轉譯 global-actor 隔離的函式型別未知）。**需要在含 `send` 的 baseline 上實測後才寫進 skill**——沒驗證過的規則不該進 skip。

  **2026-09-17 補：候選專案的形狀比本條原本寫的更深。** `FoodEntropy` 實測 4 個 `let send` 宣告、7 個 leading-dot 呼叫點（`Settings` 與 `BucketList` 兩個 feature）。完整路徑是 `SettingsView` → `private extension` → `struct PurchaseSection` → `enum Action` → `.removeAdsDidTap`，**中間那層在 `private extension` 裡**；而且 owning type 不在方法簽章上——Skip 得穿透 stored property `let send: (Action) -> Void` 的**參數型別**才推得出 `Action`。這比眉角 #6 記的「巢狀 leading-dot」更間接，是「經由 closure 型別的巢狀 leading-dot」，demo 沒有這個形狀。另外該專案的型別是 `(Action) -> Void`、**沒有 `@MainActor`**，所以就算移植了也只解鎖一半：巢狀限定驗得到，global-actor 隔離的函式型別怎麼轉譯仍然未知。**以上是待驗證的形狀，不是規則，不得寫進 `mvvmc-skip`。** 另外這兩件事是**反向耦合**的：補上 `@MainActor` 會讓專案對齊規範，但 Skip 要轉譯的東西就從普通函式型別變成 global-actor 隔離的函式型別——正是未驗證的那一半；不補則維持規範落後但轉譯形狀單純。**哪個先做會改變另一個的難度，真要移植得先決定順序。**

  **2026-09-11 追記：整份 `mvvmc-skip` 已標為凍結的筆記**，理由當時寫的是「從未被任何 Android 建置驗證過」。**2026-09-17 更正：那句是錯的**——`MVVMC-Skip` 已在 skip 1.9.3 上跑完 M0–M21 並於模擬器渲染與端到端驗證，詳見 `mvvmc-skip` 凍結段。凍結維持，但理由改成「驗證綁在 `v1.1.0` baseline 上、會過期而沒有機制會發現」。按 `Experiments/README.md` 的標準（依豁免密度排風險，不依主題），它仍是風險最高的一份。**所以這條 TODO 仍是「解除凍結的條件之一」，只是條件從『找到一個 Android 專案』變成『把驗證拉到當前 baseline』。**
